...
@SmallTest
class GetPersonImagesApiTest : BaseNetworkTest() {
    ...
    override fun setUp() {
        setMockServerUrl()
        mockedResponse = MockResponseFileReader("raw/get_person_images_response.json").content
    ...
    fun `given mock response with 200 code verify that getPerson response is not null or empty list`() {
        ...
        runBlocking {
            val mockResponseList = mockService.getPersonImages("37625")
    ...
    fun `given mock response with 200 code verify that getPerson response is the expected`() {
        ...
        runBlocking {
            val mockResponse = mockService.getPersonImages("37625")

            Assert.assertTrue(mockResponse.isSuccessful)
            Assert.assertEquals(37625L, mockResponse.body()?.id)
            Assert.assertNotNull(mockResponse.body()?.profiles)
            Assert.assertEquals("/h0A3pzHaNTVRD7xNfabuoyadJba.jpg", mockResponse.body()?.profiles?.getOrNull(0)?.filePath)
        }
    }
